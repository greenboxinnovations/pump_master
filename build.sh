	...
	then
		...
		if [ -z "$2" ]
			then
				...
				g++ -g -std=c++0x VlcCap.cpp $1 -o $a `pkg-config --cflags --libs opencv4 libvlc` -lpthread  -lmysqlcppconn 
			else
				# g++ -std=c++0x $1 -o $2 `pkg-config --cflags --libs opencv` -lpthread  -lmysqlcppconn
				g++ -g -std=c++0x $1 -o $2 `pkg-config --cflags --libs opencv4 libvlc` -lpthread  -lmysqlcppconn
		fi 	
